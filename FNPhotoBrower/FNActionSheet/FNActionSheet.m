...
CGFloat const kFNActionSheetSectionHeight = 10; ///< 分区间距

@interface FNActionSheet () <UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, copy) NSString *title;
@property (nonatomic, copy) NSString *cancelTitle;
@property (nonatomic, copy) NSArray<FNActionSheetItem *> *items;
@property (nonatomic, copy) FNActionSheetHandler selectedHandler;
...
static NSString * const kFNActionSheetCellIdentifier = @"kFNActionSheetCellIdentifier";

@implementation FNActionSheet
...
- (instancetype)initWithTitle:(NSString *)title delegate:(id<FNActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle highlightedButtonTitle:(NSString *)highlightedButtonTitle otherButtonTitles:(NSArray<NSString *> *)otherButtonTitles
{
    ...
    for (NSString *otherTitle in otherButtonTitles) {
        if (otherTitle && otherTitle.length > 0) {
            [titleItems addObject:FNActionSheetTitleItemMake(FNActionSheetTypeNormal, otherTitle)];
        }
    }
    // 高亮按钮, 高亮按钮放在最下面.
    if (highlightedButtonTitle && highlightedButtonTitle.length > 0) {
        [titleItems addObject:FNActionSheetTitleItemMake(FNActionSheetTypeHighlighted, highlightedButtonTitle)];
    }
...
- (void)commonInit
{
    self.backgroundColor = FNActionSheetColorWithString(FNActionSheetBackColor);
    self.translatesAutoresizingMaskIntoConstraints = NO; // 允许约束
    
    _contentAlignment = FNContentAlignmentCenter; // 默认样式为居中
...
{
    CGFloat labelHeight = [_titleLabel.attributedText boundingRectWithSize:CGSizeMake([self currentScreenWidth] - FNActionSheetDefaultMargin*2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading context:nil].size.height;
    CGFloat headerHeight = ceil(labelHeight)+FNActionSheetDefaultMargin*2;
...
- (CGFloat)contentHeight
{
    ...
    CGFloat rowHeightSum = (_items.count+1)*FNActionSheetRowHeight+kFNActionSheetSectionHeight;
...
{
    CGFloat contentMaxHeight = [self currentScreenHeight] * FNActionSheetContentMaxScale;
    ...
    if (orientation == UIInterfaceOrientationLandscapeLeft  || orientation == UIInterfaceOrientationLandscapeRight) {
        bottomHeight = FNActionSheetLandscapeBottomHeight();
    } else {
        bottomHeight = FNActionSheetPortraitBottomHeight();
    }
...
- (void)updateTitleAttributeText
{
    ...
    UIColor *titleTextColor = FNActionSheetColorWithString(FNActionSheetTitleColor);
    CGFloat  lineSpacing = FNActionSheetTitleLineSpacing;
    CGFloat  kernSpacing = FNActionSheetTitleKernSpacing;
...
- (void)showWithSelectedCompletion:(FNActionSheetHandler)selectedHandler
{
    ...
    // 适配屏幕高度
    CGFloat contentMaxHeight = CGRectGetHeight(self.popupWindow.frame)*FNActionSheetContentMaxScale+bottomHeight;
    if (contentHeight > contentMaxHeight) {
        self.tableView.scrollEnabled = YES;
        contentHeight = contentMaxHeight;
    }
...
- (UIWindow *)popupWindow
{
    ...
    _popupWindow.windowLevel = MAXFLOAT+1;//UIWindowLevelStatusBar+1;
...
- (UITableView *)tableView
{
    ...
    [_tableView registerClass:[FNActionSheetCell class] forCellReuseIdentifier:kFNActionSheetCellIdentifier];
...
- (UIView *)headerView
{
    ...
    headerView.backgroundColor = FNActionSheetColorWithString(FNActionSheetRowNormalColor);
    ...
    // 标题内容边距 (ps: 要修改这个边距不要在这里修改这个labelMargin, 要到配置类中修改 FNActionSheetDefaultMargin, 不然可能出现界面适配错乱).
    CGFloat labelMargin = FNActionSheetDefaultMargin;
    // 计算内容高度
...
{
    return FNActionSheetRowHeight;
}

- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    return (section == 1)?kFNActionSheetSectionHeight:CGFLOAT_MIN;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    FNActionSheetCell *cell = [tableView dequeueReusableCellWithIdentifier:kFNActionSheetCellIdentifier];
    return cell;
}

- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    FNActionSheetCell *sheetCell = (FNActionSheetCell *)cell;
    if (indexPath.section == 0) {
        ...
        // 默认取消的单元格没有附带icon
        FNActionSheetItem *cancelItem = FNActionSheetTitleItemMake(FNActionSheetTypeNormal, _cancelTitle);
        // 如果其它单元格中附带icon的话则添加上默认的取消icon.
        for (FNActionSheetItem *item in _items) {
            if (item.image) {
                cancelItem = FNActionSheetTitleWithImageItemMake(FNActionSheetTypeNormal, [UIImage imageNamed:@"FNActionSheet_cancel"], _cancelTitle);
                break;
            }
        }
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1*NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self hideWithCompletion:^{
            if (indexPath.section == 0) {
                ...
                if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(FNActionSheet:selectedIndex:)]) {
                    [weakSelf.delegate FNActionSheet:self selectedIndex:indexPath.row];
                }
            }
        }];
    });
