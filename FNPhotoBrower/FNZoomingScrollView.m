...
@interface FNZoomingScrollView () <UIScrollViewDelegate>
{
   UIScrollView *_scrollview;
}

@property (nonatomic , strong) UIImageView  *photoImageView;
@property (nonatomic , strong) FNProgressView *progressView;
...
@implementation FNZoomingScrollView
...
- (UILabel *)stateLabel
{
    if (_stateLabel == nil) {
        ...
        _stateLabel.text = FNPhotoBrowserLoadNetworkImageFail;
...
- (FNProgressView *)progressView
{
    if (_progressView == nil) {
        _progressView = [[FNProgressView alloc] init];
    }
...
- (void)layoutSubviews
{
    ...
    self.progressView.fn_centerX = self.fn_width * 0.5;
    self.progressView.fn_centerY = self.fn_height * 0.5;
...
- (void)setShowHighQualityImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder
{
    ...
    self.progressView.mode = FNProgressViewProgressMode;
    self.imageURL = url;
    ...
    [weakSelf.photoImageView sd_setImageWithURL:url placeholderImage:placeholder options:SDWebImageRetryFailed| SDWebImageLowPriority| SDWebImageHandleCookies progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (expectedSize > 0) { //[strongSelf.imageURL isEqual:targetURL]
...
//                NSLog(@"targetURL %@ , strongSelf %@ , strongSelf.imageURL = %@ , progress = %f",targetURL , strongSelf , strongSelf.imageURL,strongSelf.progress);
        ...
        if (error) {
            ...
            FNPBLog(@"加载图片失败 , 图片链接imageURL = %@ , 错误信息: %@ ,检查是否开启允许HTTP请求",imageURL,error);
        } else {
...
- (void)setMaxAndMinZoomScales
{
    ...
    self.photoImageView.fn_width = self.fn_width;
    self.photoImageView.fn_height = self.fn_width / imageWidthHeightRatio;
    self.photoImageView.fn_x = 0;
    if (self.photoImageView.fn_height > FNScreenH) {
        self.photoImageView.fn_y = 0;
        self.scrollview.scrollEnabled = YES;
    } else {
        self.photoImageView.fn_y = (FNScreenH - self.photoImageView.fn_height ) * 0.5;
        self.scrollview.scrollEnabled = NO;
    }
    self.scrollview.maximumZoomScale = MAX(FNScreenH / self.photoImageView.fn_height, 3.0);
    self.scrollview.minimumZoomScale = 1.0;
    self.scrollview.zoomScale = 1.0;
    self.scrollview.contentSize = CGSizeMake(self.photoImageView.fn_width, MAX(self.photoImageView.fn_height, FNScreenH));
}
