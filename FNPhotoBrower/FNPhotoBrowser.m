...
#import "FNZoomingScrollView.h"
#import "TAPageControl.h"
#import "FNActionSheetConfig.h"
#import "FNActionSheet.h"
#import "FNPhotoBrowserConfig.h"

#define BaseTag 100

@interface FNPhotoBrowser () <FNZoomingScrollViewDelegate , UIScrollViewDelegate>
...
- (UIImage *)placeholderImage
{
    if (!_placeholderImage) {
        _placeholderImage = [UIImage fn_imageWithColor:[UIColor grayColor] size:CGSizeMake(100, 100)];
    }
...
//        _photoBrowserWindow.windowLevel = UIWindowLevelAlert;//2000的优先级,这样不会遮盖UIAlertView的提示弹框
        ...
        tempVC.view.backgroundColor = FNPhotoBrowserBackgrounColor;
;
        ...
        //        BOOL windowSizeIsEqualToScreen = (window.fn_width == FNScreenW && window.fn_height == FNScreenH);
        //        if(windowOnMainScreen && windowIsVisible && windowLevelSupported && windowSizeIsEqualToScreen) {
        //            return window;
        //        }
        //    }
        //
        //    FNPBLog(@"FNPhotoBrowser在当前工程未匹配到合适的window,请根据工程架构酌情调整此方法,匹配最优窗口");
        //    if (FNPhotoBrowserDebug) {
        //        NSAssert(false, @"FNPhotoBrowser在当前工程未匹配到window,请根据工程架构酌情调整findTheMainWindow方法,匹配最优窗口");
        //    }
        //
...
- (void)setPageControlStyle:(FNPhotoBrowserPageControlStyle)pageControlStyle
{
    FNLogFunc;
...
- (void)setBrowserStyle:(FNPhotoBrowserStyle)browserStyle
{
    _browserStyle = browserStyle;
    [self updateIndexVisible];
}

- (void)setPageControlAliment:(FNPhotoBrowserPageControlAliment)pageControlAliment
{
    _pageControlAliment = pageControlAliment;
    switch (self.pageControlAliment) {
        case FNPhotoBrowserPageControlAlimentLeft:
        {
            self.pageControl.fn_x = 10;
        }
            break;
        case FNPhotoBrowserPageControlAlimentRight:
        {
            self.pageControl.fn_x = (self.fn_width - self.pageControl.fn_width) - 10;
        }
            break;
        case FNPhotoBrowserPageControlAlimentCenter:
        default:
        {
            self.pageControl.fn_x = (self.fn_width - self.pageControl.fn_width) * 0.5;
        }
            break;
    }
}
...
- (void)initial
{
    self.backgroundColor = FNPhotoBrowserBackgrounColor;
    self.visibleZoomingScrollViews = [[NSMutableSet alloc] init];
    self.reusableZoomingScrollViews = [[NSMutableSet alloc] init];
    [self placeholderImage];
    
    _pageControlAliment = FNPhotoBrowserPageControlAlimentCenter;
    _pageControlDotSize = CGSizeMake(10, 10);
    _pageControlStyle = FNPhotoBrowserPageControlStyleAnimated;
    _hidesForSinglePage = YES;
    _currentPageDotColor = [UIColor whiteColor];
    _pageDotColor = [UIColor lightGrayColor];
    _browserStyle = FNPhotoBrowserStylePageControl;
...
- (void)iniaialUI
{
    ...
    if (self.currentImageIndex == 0) { // 如果刚进入的时候是0,不会调用scrollViewDidScroll:方法,不会展示第一张图片
        FNFormatLog(@"self.currentImageIndex == %zd",self.currentImageIndex);
        [self showPhotos];
    }
...
- (void)setUpPageControl
{
    ...
    switch (self.pageControlStyle) {
        case FNPhotoBrowserPageControlStyleAnimated:
        {
            ...
            break;
        case FNPhotoBrowserPageControlStyleClassic:
        {
...
- (void)orientationDidChange
{
    ...
    FNZoomingScrollView *temp = [self zoomingScrollViewAtIndex:self.currentImageIndex];
...
- (void)updateFrames
{
    ...
    rect.size.width += FNPhotoBrowserImageViewMargin;
    self.scrollView.frame = rect; // frame修改的时候,也会触发scrollViewDidScroll,不是每次都触发
    self.scrollView.fn_x = 0;
    self.scrollView.contentSize = CGSizeMake((self.scrollView.fn_width) * self.imageCount, 0);
    self.scrollView.contentOffset = CGPointMake(self.currentImageIndex * (self.scrollView.fn_width), 0);// 回触发scrollViewDidScroll
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.tag >= BaseTag) {
            obj.frame = CGRectMake((self.scrollView.fn_width) * (obj.tag - BaseTag), 0, self.fn_width, self.fn_height);
        }
    }];
    
    self.saveButton.frame = CGRectMake(30, self.fn_height - 70, 50, 25);
    self.indexLabel.bounds = CGRectMake(0, 0, 80, 30);
    self.indexLabel.fn_centerX = self.fn_width * 0.5;
    self.indexLabel.fn_centerY = 35;
    self.indexLabel.layer.cornerRadius = self.indexLabel.fn_height * 0.5;
    
    self.savaImageTipLabel.layer.cornerRadius = 5;
    self.savaImageTipLabel.clipsToBounds = YES;
    [self.savaImageTipLabel sizeToFit];
    self.savaImageTipLabel.fn_height = 30;
    self.savaImageTipLabel.fn_width += 20;
    ...
    switch (self.pageControlAliment) {
        case FNPhotoBrowserPageControlAlimentCenter:
        {
            x = (self.fn_width - size.width) * 0.5;
        }
            break;
        case FNPhotoBrowserPageControlAlimentLeft:
        {
        ...
        case FNPhotoBrowserPageControlAlimentRight:
        {
            x = self.fn_width - size.width - 10;
        }
            break;
        default:
            break;
    }
    CGFloat y = self.fn_height - size.height - 10;
...
{
    FNZoomingScrollView *currentZoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (longPress.state == UIGestureRecognizerStateBegan) {
        FNPBLog(@"UIGestureRecognizerStateBegan , currentZoomingScrollView.progress %f",currentZoomingScrollView.progress);
        ...
        FNActionSheet *actionSheet = [[FNActionSheet alloc] initWithTitle:self.actionSheetTitle delegate:nil cancelButtonTitle:self.actionSheetCancelTitle highlightedButtonTitle:self.actionSheetDeleteButtonTitle otherButtonTitles:self.actionOtherButtonTitles];
...
- (void)saveImage
{
    FNZoomingScrollView *zoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (zoomingScrollView.progress < 1.0) {
        self.savaImageTipLabel.text = FNPhotoBrowserLoadingImageText;
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    [self addSubview:self.savaImageTipLabel];
    if (error) {
        self.savaImageTipLabel.text = FNPhotoBrowserSaveImageFailText;
    } else {
        self.savaImageTipLabel.text = FNPhotoBrowserSaveImageSuccessText;
    }
    [self.savaImageTipLabel performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:1.0];
}
...
- (void)showPhotos
{
    ...
    for (FNZoomingScrollView *zoomingScrollView in self.visibleZoomingScrollViews) {
...
{
    for (FNZoomingScrollView* view in self.visibleZoomingScrollViews) {
 ...
 *  @param index 指定位置索引
 */
- (FNZoomingScrollView *)zoomingScrollViewAtIndex:(NSInteger)index
{
    for (FNZoomingScrollView* zoomingScrollView in self.visibleZoomingScrollViews) {
    ...
    FNZoomingScrollView* zoomingScrollView = [self dequeueReusableZoomingScrollView];
    [self setUpImageForZoomingScrollViewAtIndex:index];
    return zoomingScrollView;
}
...
- (void)setUpImageForZoomingScrollViewAtIndex:(NSInteger)index
{
    ...
    zoomingScrollView.frame = CGRectMake((self.scrollView.fn_width) * index, 0, self.fn_width, self.fn_height);
...
/**
 *  从缓存池中获取一个FNZoomingScrollView对象
 */
- (FNZoomingScrollView *)dequeueReusableZoomingScrollView
{
    FNZoomingScrollView *photoView = [self.reusableZoomingScrollViews anyObject];
    if (photoView) {
        [self.reusableZoomingScrollViews removeObject:photoView];
    } else {
        photoView = [[FNZoomingScrollView alloc] init];
    }
    return photoView;
}
...
- (NSURL *)highQualityImageURLForIndex:(NSInteger)index
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(photoBrowser:highQualityImageURLForIndex:)]) {
        ...
        if (!url) {
            FNPBLog(@"高清大图URL数据 为空,请检查代码 , 图片索引:%zd",index);
            return nil;
        }
        ...
        if (![url isKindOfClass:[NSURL class]]) {
            ...
            FNPBLog(@"高清大图URL数据有问题,不是NSString也不是NSURL , 错误数据:%@ , 图片索引:%zd",url,index);
            return nil;
        }
        return url;
    } else if(self.images.count>index) {
...
- (void)showFirstImage
{
    ...
    if (!self.sourceImageView) {
        if(self.datasource && [self.datasource respondsToSelector:@selector(photoBrowser:sourceImageViewForIndex:)]) {
            ...
            FNPBLog(@"需要提供源视图才能做弹出/退出图片浏览器的缩放动画");
            return;
        }
    }
    ...
    CGFloat width = FNScreenW;
    CGFloat height = FNScreenW / imageWidthHeightRatio;
    CGFloat x = 0;
    CGFloat y;
    if (height > FNScreenH) {
        y = 0;
    } else {
        y = (FNScreenH - height ) * 0.5;
    }
    ...
    [UIView animateWithDuration:FNPhotoBrowserShowImageAnimationDuration animations:^{
...
#pragma mark    -   FNZoomingScrollViewDelegate

/**
 *  单击图片,退出浏览
 */
- (void)zoomingScrollView:(FNZoomingScrollView *)zoomingScrollView singleTapDetected:(UITapGestureRecognizer *)singleTap
{
    ...
    tempView.frame = CGRectMake( - zoomingScrollView.scrollview.contentOffset.x + zoomingScrollView.imageView.fn_x,  - zoomingScrollView.scrollview.contentOffset.y + zoomingScrollView.imageView.fn_y, zoomingScrollView.imageView.fn_width, zoomingScrollView.imageView.fn_height);
    [self addSubview:tempView];
    
    [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:UIStatusBarAnimationFade];
    [UIView animateWithDuration:FNPhotoBrowserHideImageAnimationDuration animations:^{
...
- (void)updateIndexVisible
{
    switch (self.browserStyle) {
        case FNPhotoBrowserStylePageControl:
        {
            ...
            break;
        case FNPhotoBrowserStyleIndexLabel:
        {
            ...
            break;
        case FNPhotoBrowserStyleSimple:
        {
            self.indexLabel.hidden = NO;
 ...
 *
 */
+ (instancetype)showPhotoBrowserWithCurrentImageIndex:(NSInteger)currentImageIndex imageCount:(NSUInteger)imageCount datasource:(id<FNPhotoBrowserDatasource>)datasource
{
    FNPhotoBrowser *browser = [[FNPhotoBrowser alloc] init];
...
// */
//- (void)showWithImageIndex:(NSInteger)index imageCount:(NSInteger)imageCount datasource:(id<FNPhotoBrowserDatasource>)datasource
//{
//    self.currentImageIndex = index;
 ...
 *  @param delegate               FNPhotoBrowserDelegate
 *  @param cancelButtonTitle      取消按钮文字
 *  @param deleteButtonTitle      删除按钮文字
 *  @param otherButtonTitles      其他按钮数组
 */
- (void)setActionSheetWithTitle:(nullable NSString *)title delegate:(nullable id<FNPhotoBrowserDelegate>)delegate cancelButtonTitle:(nullable NSString *)cancelButtonTitle deleteButtonTitle:(nullable NSString *)deleteButtonTitle otherButtonTitles:(nullable NSString *)otherButtonTitle, ...
{
...
#pragma mark    -   public method  -->  FNPhotoBrowser简易使用方式:一行代码展示
...
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex
{
    if (images.count <=0 || images ==nil) {
        FNPBLog(@"一行代码展示图片浏览的方法,传入的数据源为空,请检查传入数据源");
        return nil;
    }
    ...
    for (id image in images) {
        if (![image isKindOfClass:[UIImage class]] && ![image isKindOfClass:[NSString class]] && ![image isKindOfClass:[NSURL class]] && ![image isKindOfClass:[ALAsset class]]) {
            FNPBLog(@"识别到非法数据格式,请检查传入数据是否为 NSString/NSURL/ALAsset 中一种");
            return nil;
        }
    }
